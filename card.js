    ...
    switch (key) {
        ...
        case 'ArrowRight': {
            if (input.selectionStart === input.value.length &&
                ...
                next.focus()
                next.selectionStart = next.value.length = 1
                next.selectionEnd = next.value.length = 1
                e.preventDefault()
            }
            break
        }
        case 'Delete': {
            if (input.selectionStart === 0 &&
                ...
                next.focus()
                next.selectionStart = next.value.length = 0
...
function onInputChange(input, newValue) {
    ...
    if (firstFour.startsWith('3')) {
        logo.src = 'amex-svgrepo-com.svg'
    } else if (firstFour.startsWith('4')) {
        logo.src = 'visa-seeklogo.com.svg'
    } else if (firstFour.startsWith('5')) {
        logo.src = 'mastercard-svgrepo-com.svg'
    } else {
        logo.s
    }
}
...
function isInput(input) {
    const parentEl = input.closest('[data-is-inputs]')
    return input.matches('input') && parentEl != null
}